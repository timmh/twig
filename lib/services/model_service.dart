...
class ModelService {
  static const int _expectedSampleCount = 160000;
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      final options = InterpreterOptions()
        ..threads =
            1 // Use single thread to reduce memory overhead
        ..useNnApiForAndroid =
            false; // Disable NNAPI to avoid additional memory

      // Load the TFLite model with optimized options
      _interpreter = await Interpreter.fromAsset(
        'weights/model_float16.tflite',
        options: options,
      );
  ...
  Future<void> _loadLabels() async {
    try {
      ...
      final labelsData = await rootBundle.loadString(
        'weights/assets/enhanced_labels.csv',
      );
      ...
      final labelData = lines
          ...
          .map((line) {
            ...
            if (parts.length >= 3) {
              ...
              return {'scientific': null, 'display': parts[0].trim()};
            } else {
              return {'scientific': null, 'display': ''};
            }
          })
          .where((data) => data['display']!.isNotEmpty)
          .toList();
      ...
      print(
        'Loaded ${_labels.length} enhanced species labels with common names',
      );
    } catch (e) {
  ...
  Float32List _prepareInputWaveform(Float32List waveform) {
    if (waveform.length > _expectedSampleCount) {
      print(
        'Trimming waveform from ${waveform.length} to $_expectedSampleCount samples',
      );
      return waveform.sublist(0, _expectedSampleCount);
    }
    return waveform;
  }
  ...
  Future<ModelInferenceResult> runInference(Float32List audioWaveform) async {
    ...
    try {
      ...
      final preparedWaveform = _prepareInputWaveform(audioWaveform);
      print(
        'Input waveform length: ${audioWaveform.length} (prepared: ${preparedWaveform.length})',
      );

      // Prepare input  - must wrap audio in list for TFLite
      final input = [preparedWaveform];
      ...
      final classificationOutput =
          outputs[classificationTensorIndex] as List<List<double>>;
      ...
      return ModelInferenceResult(topPredictions: predictions, allScores: []);
    } catch (e) {
  ...
  List<SpeciesPrediction> _getTopPredictionsOptimized(
    Float32List classificationOutput,
  ) {
    ...
    final maxIndex = scores.length < _labels.length
        ? scores.length
        : _labels.length;
    ...
    if (top1Index >= 0) {
      result.add(
        SpeciesPrediction(
          ...
          scientificName: top1Index < _scientificNames.length
              ? _scientificNames[top1Index]
              : null,
    ...
    if (top2Index >= 0) {
      result.add(
        SpeciesPrediction(
          ...
          scientificName: top2Index < _scientificNames.length
              ? _scientificNames[top2Index]
              : null,
    ...
    if (top3Index >= 0) {
      result.add(
        SpeciesPrediction(
          ...
          scientificName: top3Index < _scientificNames.length
              ? _scientificNames[top3Index]
              : null,
...
class ModelInferenceResult {
  ...
  ModelInferenceResult({required this.topPredictions, required this.allScores});
}
